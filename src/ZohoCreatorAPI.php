<?php
/**
 * A PSR-4 PHP class to consume Zoho Creator's API.
 *
 ...
 * - update
 *
 * Please read the parseResponse() method to understand the output
...
class ZohoCreatorAPI
{
     ...
     * @var  string
     */
    protected $api_ticket = NULL;

    /**
     * Zoho  API URL
     * @var  string
     */
    protected $api_url = 'https://creator.zoho.com/api/';
     ...
     * @return  none
     */
    public function __construct($login_id, $password, $api_key, $application = NULL)
    {
    ...
    public function initAPITicket()
    {
        ...
        $response = $this->parseResponse($response, 'api_ticket');

        // Set the API ticket.
        $this->api_ticket = ($response['success'] === TRUE) ? $response['api_ticket'] : NULL;
    ...
    public function killAPIticket()
    {
        ...
        ($response['success'] === TRUE) AND $this->api_ticket = NULL;
    ...
    public function add($form, $data)
    {
        ...
        // Set the request URL.
        $url = $this->api_url.'xml/'.$this->application.'/'.$form.'/add/';

        // Execute the request and parse the response.
        $response = $this->request($url, $data);
        $response =	simplexml_load_string($response);
        $response = $this->parseResponse($response, 'add');
    ...
    public function update($form, $data, $criteria, $reloperator = 'AND')
    {
        ...
        // Set the request URL.
        $url = $this->api_url.'xml/'.$this->application.'/'.$form.'/update/';

        // Execute the request and parse the response.
        $response = $this->request($url, $data);
        $response = simplexml_load_string($response);
        $response = $this->parseResponse($response, 'update');
     ...
     * @return  array   parsed response
     */
    public function updateElseAdd($form, $data, $criteria)
    {
        // Try updating.
     ...
     * @return  array   parsed response
     */
    protected function parseResponse($response, $type)
    {
        ...
        if ($response) {
            ...
            if ($type == 'add' OR $type == 'update') {
                ...
                // Include error list with code.
                } elseif (isset($response->errorlist->error->code)) {
